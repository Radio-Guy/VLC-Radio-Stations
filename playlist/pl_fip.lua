...
function probe()
	...
	and string.match(vlc.path, "^www%.radiofrance%.fr/fip/api/live/webradios.-") 
	)
end
...
function parse()
	...
	if "www.radiofrance.fr/fip/api/live/webradios" == vlc.path then
		local html = vlc.read(100000)

		for js in html:gmatch '{"id":"%d+","slug".-]}}' do
			table.insert( tracks, {
					path = vlc.access .. "://" .. vlc.path .. "?station=" .. js:match('"station":"([^"]+)"'),
					title = js:match('"legend":"([^"]+)"'):gsub(" logo", " (live)"):gsub(" Webradio", ""):gsub(" FR", ""),
					arturl = "https://raw.githubusercontent.com/Radio-Guy/VLC-Radio-Stations/main/gfx/" .. js:match('"station":"([^"]+)"') .. ".jpg"
			} )
		end
	elseif string.match(vlc.path, "www%.radiofrance%.fr/fip/api/live/webradios%?station%=") then
		local js = vlc.read(100000)
		local strg = string.match(js, '"station":"' .. string.match(vlc.path, "www%.radiofrance%.fr/fip/api/live/webradios%?station%=(.+)") .. '".-]}}')

		for url in strg:gmatch '{"url".-}' do
			table.insert( tracks, {
					path = url:match( '{"url":"(.-)"' ), 
					title = url:match( '"format":"(.-)"' ):upper() .. " / "  .. url:match( '"bitrate":(%d+)' ):gsub("^0", "192(?)") .. " kbps",
					album = strg:match('"legend":"([^"]+)"'):gsub(" logo", " (live)"):gsub(" Webradio", ""):gsub(" FR", ""),
					nowplaying = strg:match( '"firstLine":"(.-)"' ) .. " - " .. strg:match( '"secondLine":"(.-)"' )
			} )
		end
	end
